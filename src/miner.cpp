...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("KRTMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("KRTMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("KRTMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("krt-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running KRTMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
